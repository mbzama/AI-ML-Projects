...
# This script tests the login functionality and API endpoints
...
function Test-AuthenticatedEndpoint {
    ...
    try {
        ...
        Write-Host " ✓ PASS (Data received)" -ForegroundColor Green
        return $true
    }
    catch {
        Write-Host " ✗ FAIL (Error: $($_.Exception.Message))" -ForegroundColor Red
...
if ($token) {
    ...
    Test-AuthenticatedEndpoint -Url "$ApiUrl/admin/vendors" -Token $token -Description "Admin Vendors API"
    Test-AuthenticatedEndpoint -Url "$ApiUrl/admin/auctions" -Token $token -Description "Admin Auctions API"
    
    # Test vendor endpoints
    Test-AuthenticatedEndpoint -Url "$ApiUrl/vendor/events/active" -Token $token -Description "Vendor Active Events API"
} else {
    Write-Host ""
    Write-Host "Skipping authenticated endpoint tests due to login failure" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "5. Browser Test Instructions..." -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "6. Development Commands..." -ForegroundColor Cyan
Write-Host ""
