...
# Test login page
try {
    Write-Host "Checking login page..." -ForegroundColor Yellow -NoNewline
    $response = Invoke-WebRequest -Uri "$frontendUrl/login" -Method GET -TimeoutSec 5 -UseBasicParsing
    if ($response.StatusCode -eq 200) {
...
try {
    Write-Host "Checking backend at $backendUrl..." -ForegroundColor Yellow -NoNewline
    $response = Invoke-WebRequest -Uri "$backendUrl/api" -Method GET -TimeoutSec 5 -UseBasicParsing
    Write-Host " ✓ Backend is running" -ForegroundColor Green
}
catch {
    Write-Host " ✗ Backend is not accessible" -ForegroundColor Red
    Write-Host "  Please start the Spring Boot application" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "3. Manual Testing Instructions..." -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "4. Development Commands..." -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  java -jar target/procure-ai-api-0.0.1-SNAPSHOT.jar" -ForegroundColor Gray

Write-Host ""
Write-Host "5. Key Changes Made to Fix Login Redirection..." -ForegroundColor Cyan
Write-Host ""
